...
namespace {
static constexpr const char *kMirSocket{"/run/user/%d/mir_socket"};
...
bool Screencast::Setup() {
    ...
    if (!mir_connection_is_valid(connection_)) {
        qWarning() << "Failed to connect to Mir server:" << mir_connection_get_error_message(connection_);
        return false;
    }

    const auto config = mir_connection_create_display_config(connection_);
    if (!config) {
        qWarning() << "Failed to create display configuration:" << mir_connection_get_error_message(connection_);
        return false;
    }
    ...
    if (!active_output) {
        qWarning() << "Failed to find a suitable display output";
        return false;
    }
    ...
    if (!spec) {
        qWarning() << "Failed to create Mir screencast specification:" << mir_screencast_get_error_message(screencast_);
        return false;
    }
    ...
    region.left = 0;
    region.top = 0;
    region.width = display_mode->horizontal_resolution;
    region.height = display_mode->vertical_resolution;
    ...
    if (num_pixel_formats == 0) {
        qWarning() << "Failed to find suitable pixel format:" << mir_connection_get_error_message(connection_);
        return false;
    }
    ...
    if (!mir_screencast_is_valid(screencast_)) {
        qWarning() << "Failed to create Mir screencast:" << mir_screencast_get_error_message(screencast_);
        return false;
    }

    buffer_stream_ = mir_screencast_get_buffer_stream(screencast_);
    if (!buffer_stream_) {
        qWarning() << "Failed to setup Mir buffer stream";
        return false;
    }
